...
public:
	...
	static std::array<uint8_t, 2> constexpr magic_number = rai::badem_network == rai::badem_networks::badem_test_network ? std::array<uint8_t, 2> ({ 'B', 'Z' }) : rai::badem_network == rai::badem_networks::badem_beta_network ? std::array<uint8_t, 2> ({ 'B', 'Y' }) : std::array<uint8_t, 2> ({ 'B', 'X' });
	uint8_t version_max;
