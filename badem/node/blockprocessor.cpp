...
badem::block_processor::block_processor (badem::node & node_a) :
generator (node_a, badem::is_test_network ? std::chrono::milliseconds (10) : std::chrono::milliseconds (500)),
stopped (false),
active (false),
...
void badem::block_processor::add (badem::unchecked_info const & info_a)
{
	...
	{
		BOOST_LOG (node.log) << "badem::block_processor::add called for hash " << info_a.block->hash ().to_string () << " with invalid work " << badem::to_string_hex (info_a.block->block_work ());
...
void badem::block_processor::process_batch (std::unique_lock<std::mutex> & lock_a)
{
	...
	while ((!blocks.empty () || !forced.empty ()) && (timer_l.before_deadline (node.config.block_processor_batch_max_time) || (node.flags.fast_bootstrap && number_of_blocks_processed < 256 * 1024)))
	{
		...
		if (force)
		{
			auto successor (node.ledger.successor (transaction, badem::uint512_union (info.block->previous (), info.block->root ())));
...
void badem::block_processor::process_live (badem::block_hash const & hash_a, std::shared_ptr<badem::block> block_a)
{
...
badem::process_return badem::block_processor::process_one (badem::transaction const & transaction_a, badem::unchecked_info info_a)
{
	...
	switch (result.code)
	{
		...
		{
			if (info_a.modified < badem::seconds_since_epoch () - 15)
			{
...
badem::process_return badem::block_processor::process_one (badem::transaction const & transaction_a, std::shared_ptr<badem::block> block_a)
{
