...
{
public:
	socket (std::shared_ptr<badem::node>);
	void async_connect (badem::tcp_endpoint const &, std::function<void(boost::system::error_code const &)>);
	...
	void close ();
	void checkup ();
	badem::tcp_endpoint remote_endpoint ();
...
/**
 * The length of every message header, parsed by badem::message::read_header ()
 * The 2 here represents the size of a std::bitset<16>, which is 2 chars long normally
 */
static const int bootstrap_message_header_size = sizeof (badem::message_header::magic_number) + sizeof (uint8_t) + sizeof (uint8_t) + sizeof (uint8_t) + sizeof (badem::message_type) + 2;

class bootstrap_client;
class pull_info
{
public:
	typedef badem::bulk_pull::count_t count_t;
	pull_info ();
...
{
public:
	bootstrap_attempt (std::shared_ptr<badem::node> node_a);
	~bootstrap_attempt ();
	void run ();
	...
	uint64_t lazy_max_pull_blocks = badem::is_test_network ? 2 : 512;
	uint64_t lazy_max_stopped = 256;
	std::mutex lazy_mutex;
	// Wallet lazy bootstrap
	std::deque<badem::account> wallet_accounts;
};
class frontier_req_client : public std::enable_shared_from_this<badem::frontier_req_client>
{
public:
	frontier_req_client (std::shared_ptr<badem::bootstrap_client>);
	~frontier_req_client ();
	void run ();
...
class bulk_pull_client : public std::enable_shared_from_this<badem::bulk_pull_client>
{
public:
...
{
public:
	bootstrap_client (std::shared_ptr<badem::node>, std::shared_ptr<badem::bootstrap_attempt>, badem::tcp_endpoint const &);
	~bootstrap_client ();
	void run ();
...
class bulk_push_client : public std::enable_shared_from_this<badem::bulk_push_client>
{
public:
	bulk_push_client (std::shared_ptr<badem::bootstrap_client> const &);
	~bulk_push_client ();
	void start ();
...
class bulk_pull_account_client : public std::enable_shared_from_this<badem::bulk_pull_account_client>
{
public:
...
{
public:
	bootstrap_initiator (badem::node &);
	~bootstrap_initiator ();
...
{
public:
	bootstrap_listener (boost::asio::io_context &, uint16_t, badem::node &);
	void start ();
	void stop ();
...
class message;
class bootstrap_server : public std::enable_shared_from_this<badem::bootstrap_server>
{
public:
...
class bulk_pull;
class bulk_pull_server : public std::enable_shared_from_this<badem::bulk_pull_server>
{
public:
...
class bulk_pull_account_server : public std::enable_shared_from_this<badem::bulk_pull_account_server>
{
public:
...
class bulk_push_server : public std::enable_shared_from_this<badem::bulk_push_server>
{
public:
	bulk_push_server (std::shared_ptr<badem::bootstrap_server> const &);
	void receive ();
	void received_type ();
	void received_block (boost::system::error_code const &, size_t, badem::block_type);
	std::shared_ptr<std::vector<uint8_t>> receive_buffer;
...
class frontier_req;
class frontier_req_server : public std::enable_shared_from_this<badem::frontier_req_server>
{
public:
