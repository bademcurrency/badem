...
std::array<uint8_t, 2> constexpr badem::message_header::magic_number;
...
{
	badem::write (stream_a, badem::message_header::magic_number);
...
void badem::message_parser::deserialize_buffer (uint8_t const * buffer_a, size_t size_a)
{
	...
	if (size_a <= max_safe_udp_message_size)
	{
		...
		if (!error)
		{
			if (badem::is_beta_network && header.version_using < badem::protocol_version_reasonable_min)
			{
...
message (header_a),
vote (std::make_shared<badem::vote> (error_a, stream_a, header.block_type ()))
{
...
size_t constexpr badem::node_id_handshake::query_flag;
size_t constexpr badem::node_id_handshake::response_flag;
...
bool badem::node_id_handshake::is_query_flag () const
{
	return header.extensions.test (query_flag);
}

void badem::node_id_handshake::set_query_flag (bool value_a)
{
	header.extensions.set (query_flag, value_a);
}

bool badem::node_id_handshake::is_response_flag () const
{
	return header.extensions.test (response_flag);
}

void badem::node_id_handshake::set_response_flag (bool value_a)
{
