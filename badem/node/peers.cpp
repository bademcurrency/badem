...
badem::endpoint badem::map_endpoint_to_v6 (badem::endpoint const & endpoint_a)
{
...
badem::peer_information::peer_information (badem::endpoint const & endpoint_a, unsigned network_version_a, boost::optional<badem::account> node_id_a) :
endpoint (endpoint_a),
...
badem::peer_information::peer_information (badem::endpoint const & endpoint_a, std::chrono::steady_clock::time_point const & last_contact_a, std::chrono::steady_clock::time_point const & last_attempt_a) :
endpoint (endpoint_a),
ip_address (endpoint_a.address ()),
...
bool badem::peer_information::operator< (badem::peer_information const & peer_information_a) const
{
	return endpoint < peer_information_a.endpoint;
}

badem::peer_container::peer_container (badem::endpoint const & self_a) :
self (self_a),
peer_observer ([](badem::endpoint const &) {}),
disconnect_observer ([]() {})
{
}

bool badem::peer_container::contacted (badem::endpoint const & endpoint_a, unsigned version_a)
{
	auto endpoint_l (badem::map_endpoint_to_v6 (endpoint_a));
	auto should_handshake (false);
	if (version_a < badem::node_id_version)
	{
	...
	else if (!known_peer (endpoint_l))
	{
		...
		if (peers.get<badem::peer_by_ip_addr> ().count (endpoint_l.address ()) < max_peers_per_ip)
		{
	...
	else
	{
		...
		{
			peers.modify (existing, [](badem::peer_information & info) {
...
bool badem::peer_container::known_peer (badem::endpoint const & endpoint_a)
{
	std::lock_guard<std::mutex> lock (mutex);
	auto existing (peers.find (endpoint_a));
...
std::deque<badem::endpoint> badem::peer_container::list_fanout ()
{
	auto peers (random_set (size_sqrt ()));
	std::deque<badem::endpoint> result;
...
std::deque<badem::endpoint> badem::peer_container::list ()
{
	std::deque<badem::endpoint> result;
	std::lock_guard<std::mutex> lock (mutex);
	...
	badem::random_pool::shuffle (result.begin (), result.end ());
	return result;
}

std::vector<badem::peer_information> badem::peer_container::list_vector (size_t count_a)
{
	...
	{
		result.resize (count_a, badem::peer_information (badem::endpoint{}, 0));
	}
	return result;
}

badem::endpoint badem::peer_container::bootstrap_peer ()
{
	badem::endpoint result (boost::asio::ip::address_v6::any (), 0);
	std::lock_guard<std::mutex> lock (mutex);
	;
	for (auto i (peers.get<4> ().begin ()), n (peers.get<4> ().end ()); i != n;)
	{
		...
		{
			result = i->endpoint;
			peers.get<4> ().modify (i, [](badem::peer_information & peer_a) {
...
boost::optional<badem::uint256_union> badem::peer_container::assign_syn_cookie (badem::endpoint const & endpoint)
{
...
bool badem::peer_container::validate_syn_cookie (badem::endpoint const & endpoint, badem::account node_id, badem::signature sig)
{
	auto ip_addr (endpoint.address ());
	assert (ip_addr.is_v6 ());
	std::unique_lock<std::mutex> lock (syn_cookie_mutex);
	...
	if (cookie_it != syn_cookies.end () && !badem::validate_message (node_id, cookie_it->second.cookie, sig))
	{
		result = false;
...
std::unordered_set<badem::endpoint> badem::peer_container::random_set (size_t count_a)
{
	std::unordered_set<badem::endpoint> result;
...
void badem::peer_container::random_fill (std::array<badem::endpoint, 8> & target_a)
{
...
std::vector<badem::peer_information> badem::peer_container::representatives (size_t count_a)
{
	std::vector<peer_information> result;
...
void badem::peer_container::purge_syn_cookies (std::chrono::steady_clock::time_point const & cutoff)
{
...
std::vector<badem::peer_information> badem::peer_container::purge_list (std::chrono::steady_clock::time_point const & cutoff)
{
	std::vector<badem::peer_information> result;
	{
		std::lock_guard<std::mutex> lock (mutex);
		...
		for (auto i (peers.begin ()), n (peers.end ()); i != n; ++i)
		{
			peers.modify (i, [](badem::peer_information & info) { info.last_attempt = std::chrono::steady_clock::now (); });
		}
...
std::vector<badem::endpoint> badem::peer_container::rep_crawl ()
{
	std::vector<badem::endpoint> result;
...
size_t badem::peer_container::size ()
{
	std::lock_guard<std::mutex> lock (mutex);
	return peers.size ();
}

size_t badem::peer_container::size_sqrt ()
{
	return (static_cast<size_t> (std::ceil (std::sqrt (size ()))));
}

std::vector<badem::peer_information> badem::peer_container::list_probable_rep_weights ()
{
	std::vector<badem::peer_information> result;
...
badem::uint128_t badem::peer_container::total_weight ()
{
...
bool badem::peer_container::empty ()
{
	return size () == 0;
}

bool badem::peer_container::not_a_peer (badem::endpoint const & endpoint_a, bool blacklist_loopback)
{
	bool result (false);
	...
	else if (badem::reserved_address (endpoint_a, blacklist_loopback))
	{
		result = true;
	}
...
bool badem::peer_container::rep_response (badem::endpoint const & endpoint_a, badem::account const & rep_account_a, badem::amount const & weight_a)
{
	...
	if (existing != peers.end ())
	{
		peers.modify (existing, [weight_a, &updated, rep_account_a](badem::peer_information & info) {
...
void badem::peer_container::rep_request (badem::endpoint const & endpoint_a)
{
	std::lock_guard<std::mutex> lock (mutex);
	auto existing (peers.find (endpoint_a));
	if (existing != peers.end ())
...
bool badem::peer_container::reachout (badem::endpoint const & endpoint_a)
{
...
bool badem::peer_container::insert (badem::endpoint const & endpoint_a, unsigned version_a, bool preconfigured_a, boost::optional<badem::account> node_id_a)
{
	assert (endpoint_a.address ().is_v6 ());
	auto unknown (false);
	...
	if (!result)
	{
		if (version_a >= badem::protocol_version_min)
		{
			std::lock_guard<std::mutex> lock (mutex);
			auto existing (peers.find (endpoint_a));
			if (existing != peers.end ())
			{
				peers.modify (existing, [node_id_a](badem::peer_information & info) {
					info.last_contact = std::chrono::steady_clock::now ();
			...
			{
				unknown = true;
				if (!result && !badem::is_test_network)
				{
					auto ip_peers (peers.get<badem::peer_by_ip_addr> ().count (endpoint_a.address ()));
				...
				if (!result)
				{
					peers.insert (badem::peer_information (endpoint_a, version_a, node_id_a));
				}
			}
		}
	}
