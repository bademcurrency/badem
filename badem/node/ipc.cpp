...
badem::error badem::ipc::ipc_config::deserialize_json (badem::jsonconfig & json)
{
...
public:
	...
	void rpc_handle_query ()
	{
		...
		auto handler (std::make_shared<badem::rpc_handler> (node, server.rpc, body, request_id_l, response_handler_l));
	...
	void read_next_request ()
	{
		...
		async_read_exactly (buffer.data (), buffer.size (), std::chrono::seconds::max (), [this_l]() {
			...
			else if (this_l->buffer[preamble_offset::encoding] == static_cast<uint8_t> (badem::ipc::payload_encoding::json_legacy))
			{
				// Length of payload
...
public:
	...
	server (server_a), config_transport (config_transport_a)
	{
		...
		if (concurrency_a > 0)
		{
			runner = std::make_unique<badem::thread_runner> (*io_ctx, concurrency_a);
		}
	}
...
class badem::ipc::dsock_file_remover final
{
public:
...
badem::ipc::ipc_server::ipc_server (badem::node & node_a, badem::rpc & rpc_a) :
...
std::shared_ptr<std::vector<uint8_t>> badem::ipc::ipc_client::prepare_request (badem::ipc::payload_encoding encoding_a, std::string const & payload_a)
{
...
std::string badem::ipc::rpc_ipc_client::request (std::string const & rpc_action_a)
{
	auto req (prepare_request (badem::ipc::payload_encoding::json_legacy, rpc_action_a));
	auto res (std::make_shared<std::vector<uint8_t>> ());

	std::promise<std::string> result_l;
	async_write (req, [this, &res, &result_l](badem::error err_a, size_t size_a) {
		// Read length
		this->async_read (res, sizeof (uint32_t), [this, &res, &result_l](badem::error err_read_a, size_t size_read_a) {
			uint32_t payload_size_l = boost::endian::big_to_native (*reinterpret_cast<uint32_t *> (res->data ()));
			// Read json payload
			this->async_read (res, payload_size_l, [&res, &result_l](badem::error err_read_a, size_t size_read_a) {
