...
const char * default_beta_peer_network = "node-beta.badem.io";
const char * default_live_peer_network = "node.badem.io";
}

badem::node_config::node_config () :
node_config (badem::network::node_port, badem::logging ())
{
}
...
receive_minimum (badem::RAW_ratio),
vote_minimum (badem::kBDM_ratio),
online_weight_minimum (60000 * badem::kBDM_ratio),
...
unchecked_cutoff_time (std::chrono::seconds (4 * 60 * 60)) // 4 hours
{
	...
	epoch_block_signer = badem::genesis_account;
	switch (badem::badem_network)
	{
		case badem::badem_networks::badem_test_network:
			enable_voting = true;
			preconfigured_representatives.push_back (badem::genesis_account);
			break;
