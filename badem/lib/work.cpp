...
bool badem::work_validate (badem::block_hash const & root_a, uint64_t work_a, uint64_t * difficulty_a)
{
	...
	return value < badem::work_pool::publish_threshold;
}
...
badem::work_pool::work_pool (unsigned max_threads_a, std::function<boost::optional<uint64_t> (badem::uint256_union const &)> opencl_a) :
ticket (0),
done (false),
opencl (opencl_a)
{
	...
	auto count (badem::is_test_network ? 1 : std::min (max_threads_a, std::max (1u, boost::thread::hardware_concurrency ())));
	...
	while (!done || !pending.empty ())
	{
		...
		if (!empty)
		{
			...
			if (ticket == ticket_l)
			{
				...
				assert (output >= badem::work_pool::publish_threshold);
...
void badem::work_pool::generate (badem::uint256_union const & root_a, std::function<void(boost::optional<uint64_t> const &)> callback_a, uint64_t difficulty_a)
{
