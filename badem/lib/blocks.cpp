...
std::string badem::to_string_hex (uint64_t value_a)
{
...
std::shared_ptr<badem::block> badem::deserialize_block (badem::stream & stream_a, badem::block_uniquer * uniquer_a)
{
...
std::shared_ptr<badem::block> badem::deserialize_block (badem::stream & stream_a, badem::block_type type_a, badem::block_uniquer * uniquer_a)
{
	...
	switch (type_a)
	{
		...
		{
			bool error (false);
			std::unique_ptr<badem::receive_block> obj (new badem::receive_block (error, stream_a));
			if (!error)
			{
				result = std::move (obj);
			}
		...
		{
			bool error (false);
			std::unique_ptr<badem::send_block> obj (new badem::send_block (error, stream_a));
			if (!error)
			{
				result = std::move (obj);
			}
		...
		{
			bool error (false);
			std::unique_ptr<badem::open_block> obj (new badem::open_block (error, stream_a));
			if (!error)
			{
				result = std::move (obj);
			}
		...
		{
			bool error (false);
			std::unique_ptr<badem::change_block> obj (new badem::change_block (error, stream_a));
			if (!error)
			{
				result = std::move (obj);
			}
		...
		{
			bool error (false);
			std::unique_ptr<badem::state_block> obj (new badem::state_block (error, stream_a));
			if (!error)
			{
				result = std::move (obj);
			}
