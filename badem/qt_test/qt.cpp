...
TEST (wallet, status)
{
	...
	system.nodes[0]->peers.insert (badem::endpoint (boost::asio::ip::address_v6::loopback (), 10000), 0);
...
TEST (history, short_text)
{
	...
	auto wallet (std::make_shared<badem_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), account));
	badem::mdb_store store (init, system.nodes[0]->config.logging, badem::unique_path ());
	ASSERT_TRUE (!init);
	...
	badem::ledger ledger (store, system.nodes[0]->stats);
	{
		...
		badem::keypair key;
		badem::send_block send (ledger.latest (transaction, badem::test_genesis_key.pub), badem::test_genesis_key.pub, 0, badem::test_genesis_key.prv, badem::test_genesis_key.pub, 0);
		...
		badem::change_block change (receive.hash (), key.pub, badem::test_genesis_key.prv, badem::test_genesis_key.pub, 0);
...
TEST (wallet, republish)
{
	...
	badem::block_hash hash;
	{
		...
		badem::send_block block (system.nodes[0]->ledger.latest (transaction, badem::test_genesis_key.pub), key.pub, 0, badem::test_genesis_key.prv, badem::test_genesis_key.pub, 0);
		hash = block.hash ();
...
TEST (wallet, seed_work_generation)
{
	...
	auto transaction (system.nodes[0]->store.tx_begin ());
	ASSERT_FALSE (badem::work_validate (system.nodes[0]->ledger.latest_root (transaction, key1), work));
}
