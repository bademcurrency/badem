...
TEST (wallet, no_key)
{
	...
	ASSERT_FALSE (init);
	badem::transaction transaction (env.tx_begin (true));
	badem::kdf kdf;
...
TEST (wallet, work)
{
	...
	while (!done)
	{
		badem::transaction transaction (system.wallet (0)->wallets.tx_begin ());
		uint64_t work (0);
...
TEST (wallet, work_generate)
{
	...
	{
		badem::transaction transaction (system.nodes[0]->wallets.tx_begin ());
