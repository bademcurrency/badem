...
TEST (node, block_store_path_failure)
{
	...
	logging.init (path);
	badem::work_pool work (std::numeric_limits<unsigned>::max (), nullptr);
	...
	{
		{
			...
			auto info (system.nodes[0]->active.roots.find (badem::uint512_union (previous, previous)));
			ASSERT_NE (system.nodes[0]->active.roots.end (), info);
			done = info->election->announcements > badem::active_transactions::announcement_min;
		}
		ASSERT_NO_ERROR (system.poll ());
	}
...
TEST (node_config, v15_v16_upgrade)
{
	...
	test_upgrade ("node.badem.io", "peering.badem.io");
...
TEST (node, fork_publish)
{
	std::weak_ptr<badem::node> node0;
	{
		...
		auto existing (node1.active.roots.find (badem::uint512_union (send1->previous (), send1->root ())));
...
TEST (node, fork_keep)
{
	...
	auto conflict (node2.active.roots.find (badem::uint512_union (genesis.hash (), genesis.hash ())));
...
TEST (node, fork_open_flip)
{
	...
	node2.block_processor.flush ();
	std::unique_lock<std::mutex> lock (node2.active.mutex);
	auto conflict (node2.active.roots.find (badem::uint512_union (open1->previous (), open1->root ())));
	ASSERT_NE (node2.active.roots.end (), conflict);
	auto votes1 (conflict->election);
	ASSERT_NE (nullptr, votes1);
	ASSERT_EQ (1, votes1->last_votes.size ());
...
TEST (node, coherent_observer)
{
	badem::system system (24000, 1);
	auto & node1 (*system.nodes[0]);
	node1.observers.blocks.add ([&node1](std::shared_ptr<badem::block> block_a, badem::account const &, badem::uint128_t const &, bool) {
		auto transaction (node1.store.tx_begin ());
...
TEST (node, DISABLED_fork_stale)
{
	...
	node2.bootstrap_initiator.bootstrap (node1.network.endpoint ());
	node2.peers.rep_response (node1.network.endpoint (), badem::test_genesis_key.pub, badem::genesis_amount);
	badem::genesis genesis;
	badem::keypair key1;
	badem::keypair key2;
...
TEST (node, rep_self_vote)
{
	...
	auto existing (active.roots.find (badem::uint512_union (block0->previous (), block0->root ())));
...
TEST (node, no_voting)
{
	...
	ASSERT_EQ (0, node1.stats.count (badem::stat::type::message, badem::stat::detail::confirm_ack, badem::stat::dir::in));
}
...
TEST (node, vote_replay)
{
	...
	{
		auto transaction (system.nodes[0]->store.tx_begin ());
		std::lock_guard<std::mutex> lock (boost::polymorphic_downcast<badem::mdb_store *> (system.nodes[0]->store_impl.get ())->cache_mutex);
...
TEST (node, confirm_quorum)
{
	...
	while (!done)
	{
		...
		{
			std::lock_guard<std::mutex> guard (system.nodes[0]->active.mutex);
			auto info (system.nodes[0]->active.roots.find (badem::uint512_union (send1->hash (), send1->hash ())));
			ASSERT_NE (system.nodes[0]->active.roots.end (), info);
			done = info->election->announcements > badem::active_transactions::announcement_min;
		}
		ASSERT_NO_ERROR (system.poll ());
...
TEST (node, local_votes_cache)
{
	...
	{
		std::lock_guard<std::mutex> lock (boost::polymorphic_downcast<badem::mdb_store *> (node.store_impl.get ())->cache_mutex);
