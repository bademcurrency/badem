...
TEST (node, node_receive_quorum)
{
	...
	rai::block_hash previous (system.nodes[0]->latest (rai::test_genesis_key.pub));
	system.wallet (0)->insert_adhoc (key.prv);
	auto send (std::make_shared<rai::send_block> (previous, key.pub, rai::genesis_amount - rai::kBDM_ratio, rai::test_genesis_key.prv, rai::test_genesis_key.pub, system.work.generate (previous)));
	system.nodes[0]->process_active (send);
	auto iterations (0);
...
{
	rai::system system (24000, 1);
	auto price1 (system.nodes[0]->price (rai::kBDM_ratio, 1));
	ASSERT_EQ (rai::node::price_max * 100.0, price1);
	auto price2 (system.nodes[0]->price (rai::kBDM_ratio * int(rai::node::free_cutoff + 1), 1));
	ASSERT_EQ (0, price2);
	auto price3 (system.nodes[0]->price (rai::kBDM_ratio * int(rai::node::free_cutoff + 2) / 2, 1));
	ASSERT_EQ (rai::node::price_max * 100.0 / 2, price3);
	auto price4 (system.nodes[0]->price (rai::kBDM_ratio * int(rai::node::free_cutoff) * 2, 1));
	ASSERT_EQ (0, price4);
}
...
TEST (node_config, serialization)
{
	...
	ASSERT_NE (config2.online_weight_quorom, config1.online_weight_quorum);
...
TEST (node, DISABLED_fork_bootstrap_flip)
{
	...
	rai::keypair key1;
	auto send1 (std::make_shared<rai::send_block> (latest, key1.pub, rai::genesis_amount - rai::kBDM_ratio, rai::test_genesis_key.prv, rai::test_genesis_key.pub, system0.work.generate (latest)));
	rai::keypair key2;
	auto send2 (std::make_shared<rai::send_block> (latest, key2.pub, rai::genesis_amount - rai::kBDM_ratio, rai::test_genesis_key.prv, rai::test_genesis_key.pub, system0.work.generate (latest)));
...
TEST (node, broadcast_elected)
{
	...
	//std::cerr << "Other: " << rep_other.pub.to_account () << std::endl;
	{
		...
		rai::send_block fund_big (node0->ledger.latest (transaction0, rai::test_genesis_key.pub), rep_big.pub, rai::kBDM_ratio * 5, rai::test_genesis_key.prv, rai::test_genesis_key.pub, 0);
		rai::open_block open_big (fund_big.hash (), rep_big.pub, rep_big.pub, rep_big.prv, rep_big.pub, 0);
		rai::send_block fund_small (fund_big.hash (), rep_small.pub, rai::kBDM_ratio * 2, rai::test_genesis_key.prv, rai::test_genesis_key.pub, 0);
		rai::open_block open_small (fund_small.hash (), rep_small.pub, rep_small.pub, rep_small.prv, rep_small.pub, 0);
		rai::send_block fund_other (fund_small.hash (), rep_other.pub, rai::kBDM_ratio * 1, rai::test_genesis_key.prv, rai::test_genesis_key.pub, 0);
...
TEST (node, DISABLED_unconfirmed_send)
{
	...
	auto send1 (wallet0->send_action (rai::genesis_account, key0.pub, 2 * rai::BDM_ratio));
	auto iterations0 (0);
	while (node1.balance (key0.pub) != 2 * rai::BDM_ratio || node1.bootstrap_initiator.in_progress ())
	{
		system.poll ();
		++iterations0;
	...
	rai::send_block send2 (latest, rai::genesis_account, rai::BDM_ratio, key0.prv, key0.pub, node0.work_generate_blocking (latest));
	{
	...
	auto send3 (wallet1->send_action (key0.pub, rai::genesis_account, rai::BDM_ratio));
	auto iterations (0);
...
TEST (node, rep_list)
{
	...
	wallet0->send_action (rai::test_genesis_key.pub, key1.pub, rai::BDM_ratio);
...
TEST (node, vote_replay)
{
	...
	system.wallet (0)->insert_adhoc (rai::test_genesis_key.prv);
	auto block (system.wallet (0)->send_action (rai::test_genesis_key.pub, key.pub, rai::kBDM_ratio));
	ASSERT_NE (nullptr, block);
	auto done (false);
	auto iterations (0);
	while (!done)
