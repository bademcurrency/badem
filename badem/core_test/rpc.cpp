...
TEST (rpc, history)
{
	...
	rai::genesis genesis;
	rai::state_block usend (rai::genesis_account, node0->latest (rai::genesis_account), rai::genesis_account, rai::genesis_amount - rai::kBADEM_ratio, rai::genesis_account, rai::test_genesis_key.prv, rai::test_genesis_key.pub, 0);
	...
	ASSERT_EQ (rai::kBADEM_ratio.convert_to<std::string> (), std::get<2> (history_l[0]));
	ASSERT_EQ (5, history_l.size ());
	...
	ASSERT_EQ (rai::kBADEM_ratio.convert_to<std::string> (), std::get<2> (history_l[1]));
...
TEST (rpc, payment_wait)
{
	...
	request1.put ("amount", rai::amount (rai::BADEM_ratio).to_string_dec ());
	...
	system.wallet (0)->send_action (rai::test_genesis_key.pub, key.pub, rai::BADEM_ratio);
	...
	request1.put ("amount", rai::amount (rai::BADEM_ratio * 2).to_string_dec ());
	test_response response3 (request1, rpc, system.service);
	while (response3.status == 0)
	{
		system.poll ();
...
TEST (rpc, badem_to_raw)
{
	...
	ASSERT_EQ (200, response1.status);
	ASSERT_EQ (rai::BADEM_ratio.convert_to<std::string> (), response1.json.get<std::string> ("amount"));
}
...
TEST (rpc, badem_from_raw)
{
	...
	request1.put ("amount", rai::BADEM_ratio.convert_to<std::string> ());
	test_response response1 (request1, rpc, system.service);
	while (response1.status == 0)
	{
		system.poll ();
...
TEST (rpc, block_create_state)
{
	...
	request.put ("balance", (rai::genesis_amount - rai::kBADEM_ratio).convert_to<std::string> ());
...
TEST (rpc, block_create_state_open)
{
	...
	rai::genesis genesis;
	system.wallet (0)->insert_adhoc (rai::test_genesis_key.prv);
	auto send_block (system.wallet (0)->send_action (rai::test_genesis_key.pub, key.pub, rai::kBADEM_ratio));
	...
	request.put ("representative", rai::test_genesis_key.pub.to_account ());
	request.put ("balance", rai::kBADEM_ratio.convert_to<std::string> ());
...
TEST (rpc, confirmation_history)
{
	rai::system system (24000, 1);
	system.wallet (0)->insert_adhoc (rai::test_genesis_key.prv);
	auto block (system.wallet (0)->send_action (rai::test_genesis_key.pub, rai::test_genesis_key.pub, rai::kBADEM_ratio));
	auto iterations (0);
	...
	ASSERT_EQ ((rai::genesis_amount - rai::kBADEM_ratio).convert_to<std::string> (), tally);
	system.stop ();
}
