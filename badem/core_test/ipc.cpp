...
TEST (ipc, asynchronous)
{
	...
	badem::ipc::ipc_server ipc (*system.nodes[0], rpc);
	badem::ipc::ipc_client client (system.nodes[0]->io_ctx);

	auto req (client.prepare_request (badem::ipc::payload_encoding::json_legacy, std::string (R"({"action": "block_count"})")));
...
TEST (ipc, synchronous)
{
	...
	system.nodes[0]->config.ipc_config.transport_tcp.port = 24077;
	badem::ipc::ipc_server ipc (*system.nodes[0], rpc);
	badem::ipc::rpc_ipc_client client (system.nodes[0]->io_ctx);
