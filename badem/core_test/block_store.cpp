...
TEST (block_store, upgrade_v4_v5)
{
	...
	auto path (rai::unique_path ());
	{
		...
		rai::keypair key0;
		rai::send_block block0 (info.head, key0.pub, rai::genesis_amount - rai::kBDM_ratio, rai::test_genesis_key.prv, rai::test_genesis_key.pub, 0);
...
TEST (block_store, upgrade_v9_v10)
{
	...
	rai::block_hash hash (0);
	{
		...
		{
			balance = balance - rai::kBDM_ratio;
	...
	ASSERT_EQ (block_info.balance.number (), rai::genesis_amount - rai::kBDM_ratio * 31);
}
