	...
	if (ec == badem::error_cli::unknown_command)
	{
		...
		else if (vm.count ("debug_profile_kdf"))
		{
			...
			while (true)
			{
				...
				auto success (argon2_hash (1, badem::wallet_store::kdf_work, 1, password.data (), password.size (), salt.bytes.data (), salt.bytes.size (), result.bytes.data (), result.bytes.size (), NULL, 0, Argon2_d, 0x10));
				(void)success;
				...
				if (!result)
				{
					...
					if (!error)
					{
						...
						if (!error)
						{
							...
							auto opencl (badem::opencl_work::create (true, { platform, device, threads }, logging));
							badem::work_pool work_pool (std::numeric_limits<unsigned>::max (), opencl ? [&opencl](badem::uint256_union const & root_a) {
		...
		else if (vm.count ("debug_profile_process"))
		{
			if (badem::is_test_network)
			{
				...
				badem::block_hash genesis_latest (node->latest (badem::test_genesis_key.pub));
			...
			{
				std::cerr << "For this test ACTIVE_NETWORK should be badem_test_network" << std::endl;
			}
		}
		...
		else if (vm.count ("debug_rpc"))
		{
			...
			badem::rpc_config rpc_config_l;
			rpc_config_l.enable_control = true;
			std::unique_ptr<badem::rpc> rpc_l = get_rpc (inactive_node_l.node->io_ctx, *inactive_node_l.node, rpc_config_l);
			std::string req_id_l ("1");
			badem::rpc_handler handler_l (*inactive_node_l.node, *rpc_l, command_l.str (), req_id_l, response_handler_l);
		...
		else if (vm.count ("version"))
		{
			...
			{
				std::cout << "Version " << BADEM_MAJOR_MINOR_VERSION << std::endl;
			}
			else
			{
				std::cout << "Version " << BADEM_MAJOR_MINOR_RC_VERSION << std::endl;
			}
		}
		else
		{
