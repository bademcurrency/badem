...
pub fn launch_node(
    ...
    let data_dir = tmp_dir.join(format!("Badem_load_test_{}", i));
    match fs::create_dir(&data_dir) {
        Ok(_) => {}
        Err(ref e) if e.kind() == io::ErrorKind::AlreadyExists => {
            let _ = fs::remove_file(data_dir.join("data.ldb"));
        }
        r => r.chain_err(|| "failed to create badem_node data directory")?,
    }
    ...
    let config = json!({
        ...
        "node": {
            ...
            "preconfigured_representatives": [
                "badem_3e3j5tkog48pnny9dmfzj1r16pg8t1e76dz5tmac6iq689wyjfpiij4txtdo"
            ],
            "inactive_supply": "0",
    ...
    let child = Command::new(badem_node)
        .arg("--data_path")
        .arg(&data_dir)
        .arg("--daemon")
        .spawn_async(&handle)
        .chain_err(|| "failed to spawn badem_node")?;
...
pub fn connect_node<C: Connect>(
    ...
    })).then(|x| x.chain_err(|| "failed to call badem_node RPC"))
            .map(|_| ()),
    ) as _
}
